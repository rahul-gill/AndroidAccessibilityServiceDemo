...
object MemoryTracker {
    ...
    private val handler: Handler = Handler(
        getAndStartHandlerThread().looper
    )
    ...
    fun poll() {
        ...
        if (currentHeapUsage.currentTotalBytes != previousAppHeadUsage.currentTotalBytes) {
            if (currentHeapUsage.currentTotalBytes > previousAppHeadUsage.currentTotalBytes) {
                Log.d(
                    TAG,
                    "The system increased our app JVM heap from ${previousAppHeadUsage.currentTotalBytes.byteDisplay()} to ${currentHeapUsage.currentTotalBytes.byteDisplay()}"
                )
            } else {
                Log.d(
                    TAG,
                    "The system decreased our app JVM heap from ${previousAppHeadUsage.currentTotalBytes.byteDisplay()} to ${currentHeapUsage.currentTotalBytes.byteDisplay()}"
                )
            }
        }
        ...
        if (currentHeapUsage.usedBytes >= previousAppHeadUsage.usedBytes) {
            ...
            Log.d(
                TAG,
                "Used memory has decreased from ${previousAppHeadUsage.usedBytes.byteDisplay()} to ${currentHeapUsage.usedBytes.byteDisplay()}"
            )
            increaseMemoryCount = 0
        }
        ...
        if (increaseMemoryCount > 0 && increaseMemoryCount % 5 == 0) {
            Log.d(
                TAG,
                "Used memory has increased or stayed the same for the last $increaseMemoryCount intervals (${increaseMemoryCount * POLLING_INTERVAL.milliseconds.inWholeSeconds} seconds). Using: ${currentHeapUsage.usedBytes.byteDisplay()}, Free: ${currentHeapUsage.freeBytes.byteDisplay()}, CurrentTotal: ${currentHeapUsage.currentTotalBytes.byteDisplay()}, MaxPossible: ${currentHeapUsage.maxPossibleBytes.byteDisplay()}"
            )
        }
    ...
    fun getDetailedMemoryStats(): DetailedMemoryStats {
        ...
        return DetailedMemoryStats(
            ...
            appNativeHeapUsageKb = debugMemoryInfo.getMemoryStat("summary.native-heap")
                ?.toLongOrNull(),
    ...
    private fun Long.byteDisplay(): String {
        ...
        return "$this (%.2f MiB)".format(bytesToMB)
    }

    private fun getAndStartHandlerThread(): HandlerThread {
        val handlerThread = HandlerThread("MemoryTracker", Process.THREAD_PRIORITY_BACKGROUND)
        handlerThread.start()
